...
function volumeStartButton(volume) {
    return `<span class="is-clickable is-size-6" @click="startVolume(volume)"><i class="icon is-small has-text-success">${SVG_PLAY}</i> Start</span>`;
}

function volumeStopButton(volume) {
    if (volume.state == "Started") {
        return `<span class="is-clickable is-size-6" @click="stopVolume(volume)"><i class="icon is-small has-text-danger">${SVG_STOP}</i> Stop</span>`;
    } else {
        return `<span class="has-text-grey is-size-6"><i class="icon is-small">${SVG_STOP}</i> Stop</span>`;
    }
}

function volumeDeleteButton(volume) {
    if (volume.state == "Started") {
        return `<span class=""><i class="icon is-small has-text-grey">${SVG_DELETE}</i> Delete</span>`;
    } else {
        return `<span class="is-clickable" @click="deleteVolume(volume)"><i class="icon is-small has-text-danger">${SVG_DELETE}</i> Delete</span>`;
    }
}
